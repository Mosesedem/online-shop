...
import { use } from "react"; // For unwrapping Promise (React 19+)
...
export default function VerificationClient({ searchParams }: ClientProps) {
  // Unwrap searchParams client-side
  const resolvedParams = use(searchParams);
  const returnTo = resolvedParams.returnTo || "/shop";
  ...
  // Check authentication and verification status (unchanged)
  useEffect(() => {
  ...
  // Local loading state (post-Suspense hydration)
  if (loading) {
    return (
  ...
  if (error) {
    return (
      <div className="max-w-md w-full bg-card rounded-md shadow-lg p-8 px-4">
